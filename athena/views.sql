...
SELECT
    date AS day,
    temperature_deviation AS temp_deviation,
    temperature_deviation - LAG(temperature_deviation, 1) OVER (ORDER BY date) AS temp_trend_deviation,
    AVG(temperature_deviation) OVER (ORDER BY date ROWS BETWEEN 6 PRECEDING AND CURRENT ROW) AS temp_dev_7day_avg,
    CASE
        WHEN ABS(temperature_deviation) > 0.5 THEN 'elevated'
        WHEN ABS(temperature_deviation) > 0.3 THEN 'mild'
        ELSE 'normal'
    END AS temp_status,
    readiness_score
FROM bio_gold.daily_readiness_performance
